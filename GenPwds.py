...
# L33t or not
LEET = True
LEET_MAP = {'a': '4', 'e': '3', 'i': '1', 'o': '0', 's': '5', 'z': '2', 'l': '!'}

# Years with 4/2 digits
YEARS4 = False
YEARS4_WITH_SEPARATORS = False
YEARS2 = True
YEARS2_WITH_SEPARATORS = True
Y4 = ["0" * (4 - len(str(x))) + str(x) for x in range (2020, 2023)]
Y2 = ["0" * (2 - len(str(x))) + str(x) for x in range (6, 7)]
...
if __name__ == "__main__":
    ...
    for word in DIC:
        ...
        if (YEARS4):
            for y4 in Y4:
                if (YEARS4_WITH_SEPARATORS):
                    tmp = [word + separator + y4 for separator in SEPARATORS]
                    new_list += tmp
                else:
                    tmp = word + y4
                    new_list += [tmp]
        if (YEARS2):
            for y2 in Y2:
                if (YEARS2_WITH_SEPARATORS):
                    tmp = [word + separator + y2 for separator in SEPARATORS]
                    new_list += tmp
                else:
                    tmp = word + y2
                    new_list += [tmp]
